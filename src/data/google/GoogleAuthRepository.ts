...
function mapGoogleUser(googleUser: any | null): User | null {
  ...
  // googleUser may be either the whole response or just the user object depending on source
  const u = (googleUser.user ?? googleUser) as any;
  if (!u) return null;
  return {
    id: String(u.id ?? u.uid ?? ""),
    name: String(u.name || "User"),
    email: u.email ?? undefined,
    photoUrl: u.photo ?? u.photoURL ?? undefined,
  };
}
