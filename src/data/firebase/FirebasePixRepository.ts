...
export class FirebasePixRepository implements PixRepository {
  ...
  private buildQr({ userId, chargeId, amount, description }: { userId: string; chargeId: string; amount?: number; description?: string; }): string {
    // Keep a simple, RN-safe payload; not full EMV QR. Avoid Buffer/btoa for RN compatibility.
    const payload = encodeURIComponent(
      JSON.stringify({ v: 1, ns: 'BR.GOV.BCB.PIX', merchant: userId, chargeId, amount: amount ?? null, description: description || null })
    );
    return 'PIXQR:' + payload;
  }

  private parseQr(qr: string): { toKey?: string; merchant?: string; amount?: number; description?: string } {
    try {
