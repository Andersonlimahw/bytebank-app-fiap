...
import { collection, query, where, orderBy, addDoc, getDocs, serverTimestamp, doc, updateDoc, deleteDoc, onSnapshot } from 'firebase/firestore';
...
export class FirebaseCardRepository implements CardRepository {
  async listByUser(userId: string): Promise<DigitalCard[]> {
    ...
    return snap.docs.map((d) => {
      ...
      const updatedAt = data.updatedAt?.toMillis ? data.updatedAt.toMillis() : Number(data.updatedAt) || undefined;
      return { id: d.id, ...data, createdAt, updatedAt } as DigitalCard;
    });
  }

  async add(card: Omit<DigitalCard, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    const db = FirebaseAPI.db;
  ...
  async update(id: string, updates: Partial<Omit<DigitalCard, 'id' | 'userId' | 'createdAt'>>): Promise<void> {
    const db = FirebaseAPI.db;
