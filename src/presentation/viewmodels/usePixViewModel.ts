...
import type { PixFavorite, PixKey, PixLimits, PixTransfer } from '../../domain/entities/Pix';
import type { PixRepository } from '../../domain/repositories/PixRepository';
import { useAuth } from '../../store/authStore';

export function usePixViewModel() {
  const di = useDI();
  const repo = useMemo(() => di.resolve<PixRepository>(TOKENS.PixRepository), [di]);
  const { user } = useAuth();
  ...
  const sendByKey = useCallback(async (toKey: string, amountCents: number, description?: string) => {
    if (!user) return;
    await repo.transferByKey({ userId: user.id, toKey, amount: amountCents, description });
    await refresh();
  }, [repo, user, refresh]);

  const payQr = useCallback(async (qr: string) => {
    if (!user) return;
    await repo.payQr({ userId: user.id, qr });
    await refresh();
  }, [repo, user, refresh]);

  const createQr = useCallback(async (amountCents?: number, description?: string) => {
    if (!user) return { id: '', qr: '' };
    const res = await repo.createQrCharge({ userId: user.id, amount: amountCents, description });
    return res;
  }, [repo, user]);

  const addKey = useCallback(async (type: PixKey['type'], value?: string) => {
    if (!user) return;
  ...
  const addFav = useCallback(async (alias: string, keyValue: string, name?: string) => {
    if (!user) return;
  ...
  const updateLimits = useCallback(async (partial: Partial<Omit<PixLimits, 'userId'>>) => {
    if (!user) return;
