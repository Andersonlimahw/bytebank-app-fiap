...
type Props = { size?: number; style?: ImageStyle | any; brand?: BrandId; mode?: ThemeMode };

export const BrandLogo: React.FC<Props> = ({ size = 96, style, brand, mode }) => {
  ...
  return (
    <View>
      {logo ? <Image source={logo} style={[styles.img, style]} /> : <Text style={styles.text}>{getBrandLogoText(b)}</Text>}
    </View>
  );
};
