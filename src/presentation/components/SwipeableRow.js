...
import { Animated, PanResponder, View, Pressable, Vibration } from 'react-native';
import { useTheme } from '../theme/theme';
import { useI18n } from '../i18n/I18nProvider';
import { makeSwipeableRowStyles } from './SwipeableRow.styles';
import Ionicons from 'react-native-vector-icons/Ionicons';
export const SwipeableRow = ({ children, onEdit, onDelete, onFullSwipeDelete, rightWidth }) => {
    ...
    const onLayout = (e) => setRowWidth(e.nativeEvent.layout.width);
    return (_jsxs(View, { style: styles.wrapper, onLayout: onLayout, children: [!!(onFullSwipeDelete || onDelete) && (_jsx(Animated.View, { style: [styles.deleteHint, { opacity: hintOpacity }], children: _jsx(Animated.View, { style: { transform: [{ scale: hintIconScale }] }, children: _jsx(Ionicons, { name: "trash", size: 24, color: theme.colors.cardText }) }) })), _jsxs(View, { style: [styles.actions, { width: actionsWidth }], children: [onEdit ? (_jsx(Pressable, { style: [styles.actionBtn, { backgroundColor: theme.colors.surface }], onPress: () => {
                            close();
                            onEdit?.();
                        }, accessibilityRole: "button", accessibilityLabel: t('common.edit'), children: _jsx(Ionicons, { name: "create-outline", size: 22, color: theme.colors.text }) })) : null, onDelete ? (_jsx(Pressable, { style: [styles.actionBtn, { backgroundColor: theme.colors.danger }], onPress: () => {
                            close();
                            onDelete?.();
                        }, accessibilityRole: "button", accessibilityLabel: t('common.delete'), children: _jsx(Ionicons, { name: "trash", size: 22, color: theme.colors.cardText }) })) : null] }), _jsx(Animated.View, { style: { transform: [{ translateX }] }, ...pan.panHandlers, children: children })] }));
};
