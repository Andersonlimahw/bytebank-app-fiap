...
type AuthState = {
  ...
  loading: boolean;
  signIn: (provider: AuthProvider, options?: { email?: string; password?: string }) => Promise<void>;
...
export const useAuthStore = create<AuthState>()(
  ...
  async signIn(provider: AuthProvider, options?: { email?: string; password?: string }) {
    const repo = useDIStore.getState().di.resolve<AuthRepository>(TOKENS.AuthRepository);
    set({ loading: true });
    try {
...
export async function initAuthStore() {
  ...
  try {
    ...
    unsubscribe = repo.onAuthStateChanged((u: User | null) => useAuthStore.setState({ user: u }));
  }
}
